...
const App: React.FC = () => {
  ...
  const handleBatchScan = async (urls: string[]) => {
    ...
    for (const url of urls) {
      try {
        const foundFiles = await scanUrlWithAI(url);
        // Using a Map to deduplicate files by URL
        const uniqueFiles = Array.from(new Map(foundFiles.map(file => [file.url, file])).values());
        setAllFiles(prevFiles => Array.from(new Map([...prevFiles, ...uniqueFiles].map(f => [f.url, f])).values()));
      } catch (e) {
        console.error(`Failed to scan ${url}:`, e);
        setError(`An error occurred while scanning ${url}.`);
        // Continue with the next URL on error
      }
    }
    setIsLoading(false);
  }
  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen flex flex-col font-sans">
      ...
      <main className="container mx-auto px-4 flex-grow flex flex-col">
        ...
        />
        {error && (
            <div className="my-4 p-4 bg-rose-900/50 border border-rose-700 text-rose-300 rounded-lg text-center">
                {error}
            </div>
        )}
        {allFiles.length > 0 ? (
